...
def test_n_most_similar(loader_static, dist):
    ...
    n = 10
    words, vectors, distances = n_most_similar(loader_static, target_word, dist=dist, n=n)
...
def test_embedding_distance_invalid_word(loader_static):
    with pytest.raises(ValueError):
        embedding_distance(loader_static, "nonexistent_word1", "nonexistent_word2", dist="cosine")