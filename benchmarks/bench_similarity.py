...
# old function
def _n_most_similar_old(loader: EmbeddingLoader, target_word: str, dist: str = 'cosine', n: int = 10) -> tuple[list[str], np.ndarray, list[float]]:
    words = loader.tokens
...
print(f"Legacy:  {np.mean(legacy_times)*1000:.1f} ms ± {np.std(legacy_times)*1000:.1f}")
