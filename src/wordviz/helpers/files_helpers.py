...
def validate_file(path: str) -> bool:
    ...
    if ext in ('.gz', '.zip'):
        raise ValueError('Compressed files are not supported. Please extract the file first.')
...
def validate_gzip(gz_path: Path) -> None:
    ...
    try:
        ...
        raise ValueError(f"Downloaded file is not a valid gzip archive: {gz_path}") from e
